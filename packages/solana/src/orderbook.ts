...
/**
 * Client for interacting with the Stratum Order Book program on Solana.
 */
export class OrderBookClient {
   ...
   * Uses solanaHash for domain separation matching on-chain.
   */
  hashOrderLeaf(order: OrderLeaf): Uint8Array {
    return hashLeaf(this.serializeOrderLeaf(order), solanaHash);
  }
  ...
  buildOrderMerkleTree(orders: OrderLeaf[]): MerkleTree {
    ...
    return new MerkleTree(leaves, solanaHash);
  }
