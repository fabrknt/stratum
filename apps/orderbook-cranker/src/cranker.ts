...
export class EpochCranker {
  ...
  async processCurrentEpoch(): Promise<void> {
    ...
    const rootHex = Buffer.from(tree.root).toString('hex').slice(0, 16);
    console.log(
      `Epoch ${epoch.epochIndex}: ${epoch.orders.length} orders, root=${rootHex}...`
    );

    // Submit root on-chain
    await this.submitEpochRoot(epoch.epochIndex, Buffer.from(tree.root), epoch.orders.length);
